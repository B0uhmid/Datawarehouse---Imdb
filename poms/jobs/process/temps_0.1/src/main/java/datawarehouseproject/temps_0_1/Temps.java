...
package datawarehouseproject.temps_0_1;
 ...
 * @status
 */
public class Temps implements TalendJob {
	...
	private final String jobName = "Temps";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Temps.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row2Struct implements routines.system.IPersistableRow<row2Struct> {
		final static byte[] commonByteArrayLock_DATAWAREHOUSEPROJECT_Temps = new byte[0];
		static byte[] commonByteArray_DATAWAREHOUSEPROJECT_Temps = new byte[0];
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_DATAWAREHOUSEPROJECT_Temps) {
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_DATAWAREHOUSEPROJECT_Temps.length) {
					if (length < 1024 && commonByteArray_DATAWAREHOUSEPROJECT_Temps.length == 0) {
						commonByteArray_DATAWAREHOUSEPROJECT_Temps = new byte[1024];
					} else {
						commonByteArray_DATAWAREHOUSEPROJECT_Temps = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_DATAWAREHOUSEPROJECT_Temps, 0, length);
				strReturn = new String(commonByteArray_DATAWAREHOUSEPROJECT_Temps, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_DATAWAREHOUSEPROJECT_Temps, 0, length);
				strReturn = new String(commonByteArray_DATAWAREHOUSEPROJECT_Temps, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:PP3AcnJWjwc1O8ytbfzWVGFafFtdfT25MZ2tIw==");
				...
				try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
					stmtCreate_tDBOutput_1.execute("CREATE TABLE `" + tableName_tDBOutput_1
							+ "`(`id_Temps` INT(0)  ,`release_date` DATETIME ,`release_year` INT(4)  ,primary key(`id_Temps`))");
				}

				String insert_tDBOutput_1 = "INSERT INTO `" + "Temps"
						+ "` (`id_Temps`,`release_date`,`release_year`) VALUES (?,?,?)";
...
// # Output table : 'Temps'
								Temps_tmp.id_Temps = Numeric.sequence("id_Temps", 1, 1);
								Temps_tmp.release_date = row1.release_date;
								Temps_tmp.release_year = row1.release_year;
								Temps = Temps_tmp;
// ###############################
...
// Start of branch "Temps"
							if (Temps != null) {
								...
								row2 = null;
								finder_tUniqRow_1.id_Temps = Temps.id_Temps;
								finder_tUniqRow_1.hashCodeDirty = true;
								if (!keystUniqRow_1.contains(finder_tUniqRow_1)) {
									KeyStruct_tUniqRow_1 new_tUniqRow_1 = new KeyStruct_tUniqRow_1();

									new_tUniqRow_1.id_Temps = Temps.id_Temps;
									...
									row2.id_Temps = Temps.id_Temps;
									row2.release_date = Temps.release_date;
									row2.release_year = Temps.release_year;
...
// Start of branch "row2"
								if (row2 != null) {
									...
									if (row2.id_Temps == null) {
										pstmt_tDBOutput_1.setNull(1, java.sql.Types.INTEGER);
									} else {
										pstmt_tDBOutput_1.setInt(1, row2.id_Temps);
									}

									if (row2.release_date != null) {
										date_tDBOutput_1 = row2.release_date.getTime();
									...
									if (row2.release_year == null) {
										pstmt_tDBOutput_1.setNull(3, java.sql.Types.INTEGER);
									} else {
										pstmt_tDBOutput_1.setInt(3, row2.release_year);
									}
...
// ###############################      
	...
	public static void main(String[] args) {
		final Temps TempsClass = new Temps();

		int exitCode = TempsClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Temps.class.getClassLoader()
					.getResourceAsStream("datawarehouseproject/temps_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = Temps.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Temps");
		}
...
/************************************************************************************************
 * 113653 characters generated by Talend Open Studio for Data Integration on the
 * 3 décembre 2024, 16:23:51 CET
 ************************************************************************************************/